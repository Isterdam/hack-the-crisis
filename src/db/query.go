...
var queries = []KVP{
	...
	KVP{K: "company/slot/get/all", V: "SELECT * FROM slots WHERE company_id=$1 ORDER BY start_time"},
	...
	KVP{K: "company/slot/get/all/available", V: "SELECT * FROM slots s WHERE s.company_id=$1 AND s.max - s.booked > 0 ORDER BY s.start_time"},
	KVP{K: "company/slot/get/betweenTime/available", V: "SELECT * FROM slots s WHERE s.start_time BETWEEN $1 AND $2 AND s.company_id = $3 AND s.max - s.booked > 0 ORDER BY s.start_time"},
	...
	KVP{K: "company/get/slotAvailability", V: `SELECT count(s.id) as avg
												...
												GROUP BY t.day::date
												ORDER BY t.day::date`},
	KVP{K: "booking/update/status", V: "UPDATE bookings b SET status=$3, status_message=$4 FROM slots s WHERE b.id=$2 AND s.company_id=$1 AND b.slot_id=s.id RETURNING b.*"},
