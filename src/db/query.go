...
var queries = []KVP{
	...
	KVP{K: "booking/getByCompanyID", V: "SELECT b.id AS booking_id, * FROM bookings b JOIN slots s ON s.id=b.slot_id WHERE s.company_id=$1 AND (s.start_time >= $2 AND s.end_time <= $3)"},
